...
import { Loader2, RefreshCw, Briefcase, Code, MapPin, Calendar, ExternalLink, Trash2 } from 'lucide-react';
...
export default function OpportunitiesPage() {
    ...
    useEffect(() => {
        // Fetch data initially even if profile is null, but re-score if profile changes
        fetchData();
    }, [profile]);
    ...
    const getTabItems = () => {
        ...
        // Jobs tab: everything else (Fresher Job, Job, etc)
            ...
            <header className="bg-white border-b border-neutral-200 sticky top-0 z-50">
                <div className="max-w-7xl mx-auto px-6 h-16 flex justify-between items-center">
                    <div className="flex items-center gap-8">
                        <Link href="/dashboard" className="text-xl font-bold text-neutral-900 tracking-tight">SPORTS</Link>
                        <nav className="hidden md:flex gap-6">
                            <Link href="/dashboard" className="text-sm font-medium text-neutral-500 hover:text-neutral-900">Dashboard</Link>
                            <Link href="/opportunities" className="text-sm font-medium text-neutral-900">Opportunities</Link>
                        </nav>
                    </div>
                    <div className="flex items-center gap-4">
                        {profile?.location && <span className="text-xs text-neutral-500 hidden sm:inline-block flex items-center gap-1"><MapPin className="w-3 h-3" /> {profile.location}</span>}
                        <button
                            onClick={handleRefresh}
                            disabled={refreshing}
                            className="flex items-center gap-2 px-4 py-2 bg-neutral-900 text-white rounded-lg text-sm font-medium hover:bg-black transition-colors disabled:opacity-70"
                        >
                    ...
                    <div className="border-b border-neutral-200">
                        <div className="flex gap-8">
                            <button
                                ...
                                className={cn("pb-4 text-sm font-medium border-b-2 transition-colors flex items-center gap-2",
                                    activeTab === 'internships' ? "border-neutral-900 text-neutral-900" : "border-transparent text-neutral-500 hover:text-neutral-700")}
                            >
                            ...
                            <button
                                ...
                                className={cn("pb-4 text-sm font-medium border-b-2 transition-colors flex items-center gap-2",
                                    activeTab === 'jobs' ? "border-neutral-900 text-neutral-900" : "border-transparent text-neutral-500 hover:text-neutral-700")}
                            >
                            ...
                            <button
                                ...
                                className={cn("pb-4 text-sm font-medium border-b-2 transition-colors flex items-center gap-2",
                                    activeTab === 'hackathons' ? "border-neutral-900 text-neutral-900" : "border-transparent text-neutral-500 hover:text-neutral-700")}
                            >
                                <Code className="w-4 h-4" /> Hackathons
                                <span className="bg-neutral-100 text-neutral-600 px-2 py-0.5 rounded-full text-xs">{hackathons.length}</span>
                            </button>
                        </div>
                    </div>
